...
function createDescription(item) {
  ...
  //Requête du nom et du prix du produit depuis l'API
...
function addQuantityToSettings(item) {
  ...
  inputQuantity.addEventListener("change", () =>
    changeQuantity(item.idProduct, inputQuantity.value, item.color)
  );

  return divQuantity;
}

//Ajout de la limite de la quantité
...
//Ajout de la possibilité de changer la quantité directement depuis la page panier
function changeQuantity(idSelect, newQuantity, colorSelect) {
  const itemSelect = productStorage.find(
    (item) => item.idProduct === idSelect && item.color === colorSelect
  );
...
async function displayTotalPrice() {
  ...
  for (let i = 0; i < itemQuantity.length; i++) {
    let price = await fetch(
      "http://localhost:3000/api/products/" + productStorage[i].idProduct
    )
      .then((resp) => resp.json())
      .then(function (data) {
        return data.price;
  ...
  for (let i = 0; i < deleteItem.length; i++) {
    deleteItem[i].addEventListener("click", () => {
      ...
      productStorage = productStorage.filter(
        (element) => element.idProduct + element.color !== productSelect
      ); //filter = extraire
...
document.getElementById("firstName").placeholder = "Marie";
document.getElementById("lastName").placeholder = "Dupont";
document.getElementById("address").placeholder = "2 rue des Pins";
document.getElementById("city").placeholder = "75000 Paris";
document.getElementById("email").placeholder = "marie@gmail.com";
...
function submitOrder(event) {
  ...
  if (invalidFirstName()) {
    ...
    error.textContent = "Veuillez saisir votre prénom avec une majuscule et 3 lettres minimum.";
  ...
  if (invalidLastName()) {
    ...
    error.textContent = "Veuillez saisir votre nom avec une majuscule.";
  ...
  if (invalidAddress()) {
    ...
    error.textContent = "Veuillez saisir votre adresse.";
  ...
  if (invalidCity()) {
    ...
    error.textContent = "Veuillez saisir un code postal suivi du nom de votre ville.";
  ...
  if (invalidEmail()) {
    ...
    error.textContent = "Veuillez saisir votre adresse mail en minuscules.";
    ...
    fetch("http://localhost:3000/api/products/order", {
      ...
      .then((data) => {
        ...
        window.location.href =
          "../html/confirmation.html" + "?orderId=" + orderId; // donc dans la barre d'adresse, il y aura "?orderId=" suivi du numero de commande"
...
function getIds() {
  //récupération de l'array comprenant les ids des produits du panier
